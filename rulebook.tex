...
In the exploration phase (see \refsec{sec:exploration-phase}) teams
need to discover the positions of their machines and report them to the
\ac{refbox}.
...
\section{Game Play}
There are different game modes within the \ac{RCLL}
(see \refsec{sec:tournament-main} and \refsec{sec:tournament-challenges}).
At the beginning of a match, the \ac{refbox} must be configured according to the
game mode specification.
The \ac{refbox} then guides the match utilizing different phases, which are
...
During the exploration phase the robots of each team have \SI{3}{\minute}
to roam the environment and report their (and only their) machines
to the \ac{refbox}.
Correctly reported machines will be rewarded with points.
...
until a report regarding this machine is sent.
...
Moving or processing workpieces at an \ac{MPS} is not allowed during the
exploration phase.
...
\subsubsection{Scheduled Machine Downtime --- Main Track}
...
\subsubsection{Online Format --- Challenge Track}
...
Replicate the \ac{RCLL} exploration phase.
